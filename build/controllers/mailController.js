...
var _mailTemplate = require("../mailTemplate.js");
...
const getItemInfo = (itemId, itemOptionName) => {
  ...
  if (itemOptionName == 'noOption') {
    ...
    itemInfo.mailTitle = itemInfo.itemOptionName.indexOf(',') == -1 ? `${itemInfo.itemName} ${itemInfo.itemOptionName}` : `${itemInfo.itemName} (${itemInfo.itemOptionName})`;
  }
...
const getMonthStr = () => {
  ...
  if (today.getDate() >= 25) monthStr = todayMonth + 1;else if (today.getDate() >= 16) monthStr = `남은 ${todayMonth}`;
  return monthStr;
};
const sendMail = async (req, res) => {
  ...
  let mailTransporter = _nodemailer.default.createTransport({
    service: 'naver',
    host: 'smtp.naver.com',
    port: 587,
    auth: {
  ...
  let details = {
    ...
    html: (0, _mailTemplate.mailText)(itemInfo.mailTitle, getMonthStr()),
